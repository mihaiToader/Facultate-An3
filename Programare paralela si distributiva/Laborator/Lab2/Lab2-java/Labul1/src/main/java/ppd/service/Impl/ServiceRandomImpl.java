...
public class ServiceRandomImpl implements ServiceRandom {
    ...
    @Override
    public double getRandomDouble(int aStart, int aEnd) {
        if (aStart > aEnd) {
            throw new IllegalArgumentException("Start cannot exceed End.");
        }
        //get the range, casting to long to avoid overflow problems
    ...
    @Override
    public ComplexNumber getRandomComplexNumebr(int aStart, int aEnd) {
        return new ComplexNumber(getRandomDouble(aStart, aEnd), getRandomDouble(aStart, aEnd));
    }
}
