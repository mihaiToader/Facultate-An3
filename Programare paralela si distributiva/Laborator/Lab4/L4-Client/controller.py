...
class Controller:
    ...
    def pay_products(self):
        ...
        for p in all:
            if p.total > 0:
                message += self.DELIMITER + str(p.code) + self.DELIMITER + str(p.total)
        print(message)
        self.send_message(message)
        msg = self.receive_response()
        print("Response: " + msg)

    def pay_products_2(self, code, quantity):
        message = self.BUY_PRODUCTS
        message += self.DELIMITER + str(code) + self.DELIMITER + str(quantity)
        print(message)
        self.send_message(message)
        msg = self.receive_response()
        print("Response: " + msg)
